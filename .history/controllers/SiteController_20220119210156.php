...
public function actionCakaController()
{
    $model = new \app\models\CakeController();

    if ($model->load(Yii::$app->request->post())) {
        if ($model->validate()) {
            if($model->save()){
                return $this->redirect(['/web/cakes2/index']);
            }
        }
    }

    return $this->render('/web/cakes2/index', [
        'model' => $model,
    ]);
}
