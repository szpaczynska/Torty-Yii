...
/*public function actionOrders()
{
    ...
    if ($model->load(Yii::$app->request->post())) {
        if ($model->validate()) {
            $model->user_id = $_POST['Orders'][Yii::$app->user->identity->user_id];
            $model->cake_id = $_POST['Orders'][$records[0]['max']];
...
public function actionOrders()
{
    ...
    if ($model->load(Yii::$app->request->post())) {
        if ($model->validate()) {
            $model->user_id = $_POST[User::findByUsername(Yii::$app->user->identity->id)]['user_id'];
            $model->cake_id = $_POST['Orders']['cake_id'];
            $model->date = $_POST['Orders']['date'];
            $model->note = $_POST['Orders']['note'];
            $model->paid = $_POST['Orders']['paid'];
            if($model->save()){
                return $this->redirect(['koszyk']);
            }
        }
...
public function actionKoszyk()
{
    ...
    return $this->render('koszyk', ['dataProvider' => $dataProvider]);
}
